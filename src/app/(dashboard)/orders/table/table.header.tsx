...
interface OrdersTableHeaderProps<TData> {
  table: Table<TData>;
}

export function OrdersTableHeader<TData>({
  table,
}: OrdersTableHeaderProps<TData>) {
  ...
  useEffect(() => {
    table.getColumn("buyerName")?.setFilterValue(debouncedSearchTerm);
  ...
  return (
    <div className="flex flex-col gap-4 items:start md:flex-row md:items-center py-4 justify-between w-full border p-2 rounded">
      <div>
        <Label className="font-bold text-base">ALL ORDERS</Label>
      </div>
